...
$logoSrc   = $tieneLogo ? base_url('organizaciones/logo/' . $org['logo_url']) : '#';
...
<div class="create-container px-3 mt-3">
    ...
    <nav class="breadcrumb-digi" aria-label="breadcrumb">
        ...
        <span class="active">Editar Organización</span>
    </nav>

    <div class="create-title">Editar Organización</div>
    <div class="create-sub">Modifica los datos de <strong><?= esc($org['nombre_org'] ?? '') ?></strong>.</div>
    ...
    <form action="<?= base_url('organizaciones/update/' . $org['id_organizacion']) ?>" method="POST" enctype="multipart/form-data" novalidate>
        <?= csrf_field() ?>
        ...
        <div class="row g-3 mb-3">
            ...
            <div class="col-md-4">
                ...
                <select class="form-select" id="tipo" name="tipo" required>
                    ...
                    foreach ($tipos as $t): ?>
                        <option value="<?= $t ?>" <?= old('tipo', $org['tipo'] ?? '') === $t ? 'selected' : '' ?>><?= $t ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="col-md-6">
                ...
                <select class="form-select" id="categoria" name="categoria" required>
                    ...
                    foreach ($categorias as $c): ?>
                        <option value="<?= $c ?>" <?= old('categoria', $org['categoria'] ?? '') === $c ? 'selected' : '' ?>><?= $c ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        ...
        <!-- ═══ DIRECCIÓN ═══ -->
        <div class="toggle-bar mt-3 <?= $tieneDireccion ? 'active' : '' ?>" onclick="toggleSeccion(this,'secDireccion')">
            <i class="bi bi-geo-alt" style="font-size:1.1rem;color:#101a61;"></i>
            <div>
                <div class="toggle-label">Dirección</div>
                <div class="toggle-desc">
                    <?= $tieneDireccion
                        ? esc(($dir['estado'] ?? '') . ' — ' . ($dir['ciudad'] ?? ''))
                        : 'Opcional — carga automática desde venezuela.js' ?>
                </div>
            </div>
        </div>

        <div class="toggle-content <?= $tieneDireccion ? 'open' : '' ?>" id="secDireccion">
            <input type="hidden" name="direccion_activa" id="hDireccion" value="<?= old('direccion_activa', $tieneDireccion ? '1' : '') ?>">
        ...
        <!-- ═══ LOGO ═══ -->
        <div class="toggle-bar mt-3 <?= $tieneLogo ? 'active' : '' ?>" onclick="toggleSeccion(this,'secLogo')">
            <i class="bi bi-image" style="font-size:1.1rem;color:#101a61;"></i>
            <div>
                <div class="toggle-label">Logo de la organización</div>
                <div class="toggle-desc">
                    <?= $tieneLogo ? 'Logo actual cargado — sube uno nuevo para reemplazar' : 'Opcional — PNG, JPG o JPEG (máx. 2MB)' ?>
                </div>
            </div>
        </div>

        <div class="toggle-content <?= $tieneLogo ? 'open' : '' ?>" id="secLogo">
            <div class="py-3 text-center">
                <div class="logo-dropzone p-4 bg-light mx-auto" id="logo-dropzone" style="max-width:320px;">
                    <div class="dz-message <?= $tieneLogo ? 'd-none' : '' ?>" id="dz-message">
                    ...
                    <img id="logo-preview" src="<?= $logoSrc ?>" alt="Previsualización"
                         class="img-fluid rounded-circle mt-3 <?= $tieneLogo ? '' : 'd-none' ?>"
                         style="width:100px; height:100px; object-fit:cover;">
                </div>
                <div class="text-muted mt-2" style="font-size:.75rem;">
                    Formatos: PNG, JPG, JPEG — Tamaño sugerido: 100×100px
                    <?php if ($tieneLogo): ?>
                        <br><span class="text-info">Si no subes un archivo nuevo, se conserva el logo actual.</span>
                    <?php endif; ?>
                </div>
            </div>
        </div>
        ...
        <div class="d-flex justify-content-between align-items-center mt-4 mb-5">
            ...
            <button type="submit" class="btn-guardar">
                <i class="bi bi-check-lg"></i> Actualizar Organización
            </button>
        </div>
