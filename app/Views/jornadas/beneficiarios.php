...
<div class="container my-4">
    ...
    <div class="d-flex align-items-center gap-3 mb-3 flex-wrap">
        <div class="benef-count">
            <img src="<?= base_url('img/beneficiario-evaluado-azul.svg') ?>" alt="">
            <span><?= $total ?? 0 ?></span>
        </div>
    ...
    <?php if (!empty($beneficiarios)): ?>
        <?php
            // Mapa de íconos: color (evaluado) y gris (pendiente)
            $iconos_color = [
        ...
        <?php foreach ($beneficiarios as $b): ?>
            <?php
                ...
                // Pesquisas evaluadas para este beneficiario
            ...
            <div class="benef-card" data-search="<?= strtolower(esc($b['apellidos'].' '.$b['nombres'].' '.($b['id_digisalud']??''))) ?>">
                <div class="benef-card-menu dropdown">
                    <button class="btn btn-link" type="button" data-bs-toggle="dropdown"><i class="bi bi-three-dots-vertical"></i></button>
                    <ul class="dropdown-menu dropdown-menu-end">
                        <li><a class="dropdown-item text-danger" href="#" onclick="confirmarRemover(<?=$jornada_id?>,<?=$b['id_beneficiario']?>)"><i class="bi bi-x-circle me-2"></i>Remover</a></li>
                    </ul>
                </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body p-0">
                <p class="px-3 pt-3 text-muted" style="font-size:.82rem;">Selecciona la pesquisa:</p>
...
function abrirEvaluar(bid,pid,nombre){
    ...
    pesquisasJornada.forEach(p=>{const info=pesquisaInfo[p];if(!info)return;const li=document.createElement('li');
    li.innerHTML=`<img src="${info.img}"><div><div class="pesq-name">${info.nombre}</div><div class="pesq-desc">${info.desc}</div></div>`;
    li.onclick=()=>{Swal.fire({icon:'info',title:info.nombre,text:'Módulo de evaluación próximamente.',confirmButtonColor:'#101a61'});bootstrap.Modal.getInstance(document.getElementById('modalEvaluar')).hide();};
    lista.appendChild(li);});
    new bootstrap.Modal(document.getElementById('modalEvaluar')).show();
}
function confirmarRemover(j,b){Swal.fire({title:'¿Remover beneficiario?',text:'Se desasociará de esta jornada',icon:'warning',showCancelButton:true,confirmButtonColor:'#dc3545',cancelButtonColor:'#6c757d',confirmButtonText:'Sí, remover',cancelButtonText:'Cancelar'}).then(r=>{if(r.isConfirmed)window.location.href=`/jornadas/${j}/desasociar/${b}`;});}
const fi=document.getElementById('filtrarBenef');if(fi){fi.addEventListener('input',function(){const t=this.value.toLowerCase();document.querySelectorAll('.benef-card').forEach(c=>{c.style.display=(c.getAttribute('data-search')||'').includes(t)?'block':'none';});});}
</script>
<?= $this->endSection() ?>