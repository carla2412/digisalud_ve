...
<?= $this->section('css') ?>
<style>
.create-container{max-width:750px;margin:0 auto}
...
.create-title{font-size:1.2rem;font-weight:600;color:#0b1b3f;margin-bottom:4px}.create-sub{font-size:.82rem;color:#6c757d;margin-bottom:1.5rem}
.id-preview{background:#f0f4ff;border:1px dashed #101a61;border-radius:10px;padding:12px 18px;margin-bottom:1.5rem}.id-preview-label{font-size:.75rem;color:#6c757d}.id-preview-val{font-family:monospace;font-size:1rem;font-weight:700;color:#101a61;letter-spacing:1px}
...
.toggle-bar{display:flex;align-items:center;gap:12px;padding:12px 16px;background:#f8f9fa;border-radius:10px;cursor:pointer;margin-bottom:8px;border:1px solid #e9ecef;user-select:none;transition:border-color .2s}.toggle-bar:hover{border-color:#101a61}.toggle-bar.active{border-color:#00D4FF;background:#f0f9ff}
.toggle-bar .toggle-label{font-size:.85rem;font-weight:600;color:#0b1b3f}.toggle-bar .toggle-desc{font-size:.72rem;color:#888}
.toggle-content{max-height:0;overflow:hidden;transition:max-height .3s ease;padding:0 4px}.toggle-content.open{max-height:800px}
.btn-guardar{background:#101a61;color:#fff;border:none;padding:12px 28px;border-radius:10px;font-size:.9rem;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:8px}.btn-guardar:hover{background:#1a2a8a}
.btn-cancelar{background:transparent;color:#6c757d;border:1px solid #dee2e6;padding:12px 24px;border-radius:10px;font-size:.9rem;text-decoration:none}.btn-cancelar:hover{background:#f8f9fa;color:#333}
.tag-container{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px;min-height:30px}
.tag-item{display:inline-flex;align-items:center;gap:4px;background:#e8eaf8;border:1px solid #101a61;border-radius:20px;padding:4px 12px;font-size:.78rem;color:#101a61;font-weight:600}
.tag-item .tag-remove{cursor:pointer;font-size:.9rem;color:#dc3545;margin-left:4px;font-weight:700}
.rep-nuevo-form{background:#f8f9fa;border:1px solid #dee2e6;border-radius:8px;padding:12px 16px;margin-top:8px}.rep-nuevo-form .form-label{font-size:.75rem}
...
.rep-selected{background:#d4edda;border:1px solid #28a745;padding:8px 12px;border-radius:8px;font-size:.82rem;margin-top:8px}
...
<?php $b=$beneficiario; $dir=$direccion; $esc=$escolaridad; $fam=$familiar; ?>
<div class="container my-4"><div class="create-container">
...
<div class="id-preview"><div class="id-preview-label">ID Digisalud</div><div class="id-preview-val" id="idPreview"><?=esc($b['id_digisalud'])?></div></div>

<form method="post" action="<?=base_url("beneficiarios/actualizar/{$b['id_beneficiario']}")?>" novalidate>
<?=csrf_field()?>

<!-- DATOS PERSONALES -->
<div class="mb-4"><div class="section-header"><i class="bi bi-person"></i> Datos personales</div>
<div class="row g-3">
...
<div class="col-md-4"><label class="form-label">Sexo *</label><select name="sexo" id="fSexo" class="form-select" required onchange="actualizarIdPreview()"><option value="">Seleccionar...</option><option value="M" <?=$b['sexo']=='M'?'selected':''?>>Masculino</option><option value="F" <?=$b['sexo']=='F'?'selected':''?>>Femenino</option></select></div>
...
<!-- DIRECCIÓN -->
<div class="toggle-bar <?=$dir?'active':''?>" onclick="toggleSeccion(this,'secDireccion')"><i class="bi bi-geo-alt" style="font-size:1.1rem;color:#101a61;"></i><div><div class="toggle-label">Dirección de residencia</div><div class="toggle-desc"><?=$dir?esc(($dir['estado']??'').' - '.($dir['ciudad']??'')):'Sin dirección'?></div></div></div>
...
<div class="col-md-6"><label class="form-label">Estado</label><select id="estado" name="estado" class="form-select" data-valor="<?=esc($dir['estado']??'')?>"><option value="">Selecciona...</option></select></div>
<div class="col-md-6"><label class="form-label">Municipio</label><select id="municipio" name="municipio" class="form-select" data-valor="<?=esc($dir['municipio']??'')?>"><option value="">Selecciona...</option></select></div>
<div class="col-md-6"><label class="form-label">Parroquia</label><select id="parroquia" name="parroquia" class="form-select" data-valor="<?=esc($dir['parroquia']??'')?>"><option value="">Selecciona...</option></select></div>
...
<!-- ESCOLARIDAD -->
<div class="toggle-bar mt-2 <?=$esc?'active':''?>" onclick="toggleSeccion(this,'secEscolaridad')"><i class="bi bi-mortarboard" style="font-size:1.1rem;color:#101a61;"></i><div><div class="toggle-label">Escolaridad</div><div class="toggle-desc"><?=$esc?esc(($esc['nombre_escuela']??'').' - '.($esc['grado']??'')):'Sin escolaridad'?></div></div></div>
...
<!-- FAMILIAR -->
<div class="toggle-bar mt-2 <?=$fam?'active':''?>" onclick="toggleSeccion(this,'secFamiliar')"><i class="bi bi-people" style="font-size:1.1rem;color:#101a61;"></i><div><div class="toggle-label">Familiar / Representante</div><div class="toggle-desc"><?=$fam?esc(($fam['relacion']??'').': '.($fam['rep_nombres']??'').' '.($fam['rep_apellidos']??'')):'Sin representante'?></div></div></div>
...
<div id="repSeleccionado"><div class="rep-selected"><i class="bi bi-check-circle-fill text-success me-1"></i><strong><?=esc($fam['rep_nombres'].' '.$fam['rep_apellidos'])?></strong> <span style="color:#888;font-size:.75rem;">(<?=esc($fam['rep_id_digisalud']??'')?>)</span><span style="cursor:pointer;float:right;color:#dc3545;" onclick="limpiarRep()">✕</span></div></div>
<?php else:?>
<div id="repSeleccionado" style="display:none;"></div>
<?php endif;?>
</div>
<div class="col-12" id="repNuevoBox" style="display:none;"><div class="rep-nuevo-form"><p style="font-size:.78rem;font-weight:600;color:#101a61;margin-bottom:8px;"><i class="bi bi-person-plus me-1"></i> Registrar representante nuevo</p><div class="row g-2"><div class="col-md-6"><label class="form-label">Nombres *</label><input type="text" name="rep_nombres" class="form-control form-control-sm"></div><div class="col-md-6"><label class="form-label">Apellidos *</label><input type="text" name="rep_apellidos" class="form-control form-control-sm"></div><div class="col-md-4"><label class="form-label">Fecha nac.</label><input type="date" name="rep_fecha_nacimiento" class="form-control form-control-sm"></div><div class="col-md-4"><label class="form-label">Sexo</label><select name="rep_sexo" class="form-select form-select-sm"><option value="M">M</option><option value="F">F</option></select></div><div class="col-md-4"><label class="form-label">Teléfono</label><input type="tel" name="rep_telefono_nuevo" class="form-control form-control-sm"></div></div></div></div>
</div></div>

<!-- ANTECEDENTES -->
<?php $tieneAnt=!empty($antClinico)||!empty($antSocio)||$usaLentes;?>
<div class="toggle-bar mt-2 <?=$tieneAnt?'active':''?>" onclick="toggleSeccion(this,'secAntecedentes')"><i class="bi bi-heart-pulse" style="font-size:1.1rem;color:#101a61;"></i><div><div class="toggle-label">Antecedentes clínicos y socioeconómicos</div><div class="toggle-desc"><?=$tieneAnt?count($antClinico)+count($antSocio).' antecedente(s)':'Sin antecedentes'?></div></div></div>
<div class="toggle-content <?=$tieneAnt?'open':''?>" id="secAntecedentes">
<div class="py-3">
<div class="chk-lentes"><div class="form-check"><input class="form-check-input" type="checkbox" name="usa_lentes" value="1" id="chkUsaLentes" <?=$usaLentes?'checked':''?>><label class="form-check-label" for="chkUsaLentes"><strong><i class="bi bi-eyeglasses me-1"></i> ¿Usa lentes correctivos?</strong></label></div></div>
<hr class="my-3">
...
const vE=$e.data('valor')||'',vM=$m.data('valor')||'',vP=$p.data('valor')||'';
if(typeof ubicaciones!=='undefined'){Object.keys(ubicaciones).forEach(e=>{$e.append(`<option value="${e}" ${e===vE?'selected':''}>${e}</option>`);});}
$e.select2({placeholder:'Selecciona...'});$m.select2({placeholder:'Selecciona...'});$p.select2({placeholder:'Selecciona...'});
if(vE&&ubicaciones[vE]){Object.keys(ubicaciones[vE]).forEach(m=>{$m.append(`<option value="${m}" ${m===vM?'selected':''}>${m}</option>`);});$m.trigger('change.select2');if(vM){(ubicaciones[vE][vM]||[]).forEach(pp=>{$p.append(`<option value="${pp}" ${pp===vP?'selected':''}>${pp}</option>`);});$p.trigger('change.select2');}}
$e.on('change',function(){const est=this.value;$m.empty().append(new Option('',''));$p.empty().append(new Option('',''));$('#ciudad').val('');Object.keys(ubicaciones[est]||{}).forEach(m=>$m.append(new Option(m,m)));$m.trigger('change.select2');});
...
let repTimer;
document.getElementById('buscarRep').addEventListener('input',function(){clearTimeout(repTimer);const q=this.value.trim();const c=document.getElementById('repResultados'),n=document.getElementById('repNuevoBox');if(q.length<2){c.innerHTML='';n.style.display='none';return;}repTimer=setTimeout(()=>{fetch(`/beneficiarios/buscarAjax?q=${encodeURIComponent(q)}`).then(r=>r.json()).then(data=>{if(data.length===0){c.innerHTML='<p style="font-size:.78rem;color:#888;">No encontrado</p>';n.style.display='block';return;}n.style.display='none';let h='';data.forEach(b=>{h+=`<div class="rep-result" onclick="seleccionarRep(${b.id_beneficiario},'${b.nombres} ${b.apellidos}','${b.id_digisalud||''}')"><strong>${b.apellidos.toUpperCase()}, ${b.nombres.toUpperCase()}</strong> <span style="color:#888;font-size:.72rem;">— ${b.id_digisalud||''}</span></div>`;});c.innerHTML=h;});},300);});
function seleccionarRep(id,nom,digi){document.getElementById('representanteId').value=id;document.getElementById('repResultados').innerHTML='';document.getElementById('buscarRep').value=nom;document.getElementById('repNuevoBox').style.display='none';document.getElementById('repSeleccionado').style.display='block';document.getElementById('repSeleccionado').innerHTML=`<div class="rep-selected"><i class="bi bi-check-circle-fill text-success me-1"></i><strong>${nom}</strong> <span style="color:#888;font-size:.75rem;">(${digi})</span><span style="cursor:pointer;float:right;color:#dc3545;" onclick="limpiarRep()">✕</span></div>`;}
...
document.querySelectorAll('#antSeleccionados input[name="antecedentes[]"],#socSeleccionados input[name="antecedentes[]"]').forEach(i=>{antSet.add(parseInt(i.value));});
function initBuscAnt(iId,rId,cId,tipo){let t;document.getElementById(iId).addEventListener('input',function(){clearTimeout(t);const q=this.value.trim();const c=document.getElementById(rId);if(q.length<2){c.innerHTML='';return;}t=setTimeout(()=>{fetch(`/beneficiarios/buscarAntecedentesAjax?q=${encodeURIComponent(q)}&tipo=${encodeURIComponent(tipo)}`).then(r=>r.json()).then(data=>{let h='';data.forEach(a=>{if(antSet.has(a.id_antecedente))return;h+=`<div class="rep-result" onclick="addAnt(${a.id_antecedente},'${a.descripcion.replace(/'/g,"\\'")}','${cId}')">${a.descripcion} <span style="color:#888;font-size:.72rem;">(${a.tipo})</span></div>`;});c.innerHTML=h||'<p style="font-size:.78rem;color:#888;">Sin resultados</p>';});},300);});}
function addAnt(id,desc,cId){if(antSet.has(id))return;antSet.add(id);const c=document.getElementById(cId);const s=document.createElement('span');s.className='tag-item';s.innerHTML=`${desc}<input type="hidden" name="antecedentes[]" value="${id}"><span class="tag-remove" onclick="rmAnt(this,${id})">×</span>`;c.appendChild(s);document.getElementById(cId==='antSeleccionados'?'antResultados':'socResultados').innerHTML='';document.getElementById(cId==='antSeleccionados'?'buscarAntecedente':'buscarSocioeconomico').value='';}
